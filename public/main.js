...
(function () {
  ...
  /* ---------- Läge (ljust/mörkt) och tema (Dev/Corp) ---------- */
  ...
  const currentTheme = () => (root.dataset.theme === "corp" ? "corp" : "dev");

  const ensureCorpFonts = () => {
    if ($("#corpFonts")) return;
  ...
  if (themeSwitch) {
    themeSwitch.addEventListener("click", () => {
      const next = currentTheme() === "corp" ? "dev" : "corp";
      if (next === "corp") {
        ensureCorpFonts();
        root.dataset.theme = "corp";
      } else {
